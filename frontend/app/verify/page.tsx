...
function VerificationBadgeStack({
  ...
  // ── REVOKED: credential is invalid — show only the revoked badge ────────────
  ...
  // ── ACTIVE: build ordered verification layer chain ──────────────────────────
  // Higher-trust layers appear first; all achieved layers are shown so the
  // full trust chain is immediately visible without clicking any dropdown.
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {/* Secondary layers — shown muted with › separator so hierarchy is clear */}
      {rest.map((b) => (
...
function ResultRow({ label, value, mono = false, link }: {
  ...
  return (
    <div className="flex gap-4 py-3.5 border-b border-white/[0.05] last:border-0">
      <span className="text-white/35 text-sm w-36 shrink-0">{label}</span>
      {link ? (
...
/* ── VerifyCore — uses useSearchParams so must be in Suspense ── */
function VerifyCore() {
  ...
  // ── Core verify — now calls server API (cached + parallel) ──────────────────
  ...
  async function doVerify(id: string, bypass = false) {
    ...
    try {
      ...
      // ── Decrypt IPFS metadata if share token present ───────────────────────
      // Uses ipfsCid from server response — no need for a second blockchain call
      if (data.result?.ipfsCid && shareToken) {
        (async () => {
          try {
      ...
      <div className="max-w-3xl mx-auto mt-8">
        ...
        {result && !loading && (
          ...
          >
            ...
            {/* Legacy credential notice */}
            {isSharedLink && credMeta !== null && !credMeta.details && (
            ...
            <div className="px-8 py-4">
              ...
              <ResultRow label={v.fieldIssuer}     value={result.issuer}    mono
                link={`https://testnet.qie.digital/address/${result.issuer}`} />
              <ResultRow label={v.fieldCandidate}  value={result.candidate} mono
                link={`https://testnet.qie.digital/address/${result.candidate}`} />
              <ResultRow label={v.fieldIssuedAt}   value={issuedDate} />
              <ResultRow label={v.fieldBlockchain} value="QIE Testnet · Chain ID 1983" />
              {result.candidatePassDid && (
            ...
            <div className="px-8 pb-8 flex items-center flex-wrap gap-3">
              <a
                href={`https://testnet.qie.digital/address/${CONTRACTS.CREDENTIAL_REGISTRY}`}
                target="_blank" rel="noopener noreferrer"
