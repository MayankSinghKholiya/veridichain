...
// ── Constants ─────────────────────────────────────────────────────────────
const ADMIN_CHAIN_ID   = 1983;
const BLOCKS_PER_DAY   = 28800n; // ~3 s/block conservative — safe for QIE testnet
...
// ── Admin whitelist (env-configurable, in addition to contract owner) ─────
...
// ── Session cache helpers (cleared on tab close — no stale-data risk) ─────
...
// ── Credential Details Panel (admin view — decrypts IPFS metadata) ───────────
    ...
    <div className={`rounded-3xl border ${borderColor} transition-all`} style={{ background: bgStyle }}>
      <div className="p-6 space-y-3">
        ...
        <InfoRow label="Candidate"
          ...
          link={`https://testnet.qie.digital/address/${req.candidate}`} mono />
    ...
    <div className="rounded-3xl border border-red-500/20 transition-all" style={{ background: "rgba(239,68,68,0.05)" }}>
      <div className="p-6 space-y-3">
        ...
        <InfoRow label="Revoked By" value={`${act.revokedBy.slice(0,10)}…${act.revokedBy.slice(-8)}`}
          link={`https://testnet.qie.digital/address/${act.revokedBy}`} mono />
        {act.reason && (
          <div className="glass rounded-xl px-4 py-3">
        ...
        {act.txHash && (
          <InfoRow label="Transaction" value={`${act.txHash.slice(0,16)}…${act.txHash.slice(-6)}`}
            link={`https://testnet.qie.digital/tx/${act.txHash}`} mono />
        )}
      </div>
    </div>
  );
...
function InstitutionCard({
  inst, slashReason, onSlashReasonChange, onVerify, onSlash, verifyState, slashState,
}: {
  inst: InstitutionData;
  slashReason: string; onSlashReasonChange: (v: string) => void;
  onVerify: () => void; onSlash: () => void;
  verifyState: "idle" | "loading"; slashState: "idle" | "loading";
}) {
  ...
  return (
    <div className={`rounded-3xl border ${borderColor} transition-all`} style={{ background: bgStyle }}>
      <div className="p-6 space-y-3">
        ...
        <InfoRow label="Wallet" value={fmtAddr(inst.address)} link={`https://testnet.qie.digital/address/${inst.address}`} mono />
        ...
        <InfoRow label="Staked" value={inst.stakedAmount > 0n ? `${Number(inst.stakedAmount) / 1e18} QIEUSD` : "0 QIEUSD"} />
        ...
        <div className="pt-3 border-t border-white/[0.06] space-y-3">
          {isPending && (
            <button onClick={onVerify} disabled={verifyState !== "idle"}
              ...
              {verifyState === "loading" ? "⏳ Verifying…" : "✅ Approve Institution"}
            </button>
          )}
          {isActive && (
            <div className="space-y-2">
              <label className="block text-white/40 text-xs font-semibold uppercase tracking-wider">
                Slash Reason <span className="text-white/20 font-normal normal-case">(required)</span>
              </label>
              ...
              <button onClick={onSlash} disabled={slashState !== "idle" || !slashReason.trim()}
                className="w-full py-2.5 rounded-xl text-sm font-semibold text-white transition-all disabled:opacity-50"
                style={{ background: "linear-gradient(135deg,#dc2626,#9f1239)", boxShadow: "0 4px 14px rgba(220,38,38,0.25)" }}>
                {slashState === "loading" ? "⏳ Slashing…" : "🚫 Slash & Remove Institution"}
              </button>
            </div>
          )}
...
export default function AdminPage() {
  ...
  // ── Activity filter, date filter, pagination ──
  ...
  // ── Load requests — parallel batches of REQ_BATCH, session-cached by count ──
  ...
  // ── Load revoke activity — date-range-filtered for speed ──────────────────
  ...
  // ── Write hooks ──
  const { writeContract: doApproveW, data: approveHash }      = useWriteContract();
  const { writeContract: doRejectW,  data: rejectHash  }      = useWriteContract();
  ...
  useEffect(() => {
    if (verifyInstOk || slashInstOk) {
      loadInstitutions(); setVerifyStates({}); setSlashStates({}); setSlashReasons({});
    }
  }, [verifyInstOk, slashInstOk]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // ── Handlers ──
  function handleApprove(reqId: number) {
  ...
  function handleReject(reqId: number) {
  ...
  function handleVerifyInst(addr: string) {
  ...
  function handleSlashInst(addr: string) {
  ...
  function handleRemoveVerifier(v: string) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      <div className="min-h-screen" style={{ background: "#020817" }}>
        ...
        <div className="pt-16 flex items-center justify-center min-h-screen px-6">
          <div className="glass rounded-3xl p-12 text-center max-w-md">
            <div style={{ width: 72, height: 72, borderRadius: 20, fontSize: 32, background: "rgba(14,165,233,0.12)", border: "1px solid rgba(14,165,233,0.2)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>🔐</div>
            <h2 className="text-white font-bold text-xl mb-2">Connect Your Wallet</h2>
            <p className="text-white/40 text-sm mb-6">Connect to access the admin dashboard.</p>
            <button onClick={() => connect({ connector: injected() })} className="btn-primary text-white px-10 py-3 rounded-2xl font-semibold">Connect Wallet</button>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen" style={{ background: "#020817" }}>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-24">
        ...
        {/* Tabs */}
        <div className="flex flex-wrap gap-2 mb-8 glass rounded-2xl p-1.5 w-fit">
          {tabs.map((t) => (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      {pendingInst.map((inst) => (
                        <InstitutionCard key={inst.address} inst={inst}
                          ...
                          onVerify={() => handleVerifyInst(inst.address)} onSlash={() => handleSlashInst(inst.address)}
                          verifyState={verifyStates[inst.address] ?? "idle"} slashState={slashStates[inst.address] ?? "idle"} />
                      ))}
                    </div>
                  </section>
                )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="glass rounded-3xl p-6 space-y-4">
              ...
              {verifierList.length === 0 ? (
                <div className="text-center py-6"><p className="text-white/25 text-sm">No additional verifiers added yet.</p></div>
              ) : verifierList.map((v) => (
            ...
            <div className="space-y-4">
              <div className="glass rounded-3xl p-6 space-y-4">
                <h2 className="text-white font-bold text-lg">Add Verifier</h2>
                <p className="text-white/40 text-sm">Verifiers can approve and reject candidate verification requests.</p>
                <div>
                  <label className="block text-white/40 text-xs font-semibold uppercase tracking-wider mb-2">Wallet Address</label>
                  <input value={newVerifierInput} onChange={(e) => setNewVerifierInput(e.target.value)}
                    placeholder="0x742d35Cc6634C0532925a3b8D4C9C2f4..."
                    className="input-field w-full rounded-xl px-4 py-3 text-white placeholder-white/20 text-sm font-mono" />
                  {verifierErr && <p className="text-red-400 text-xs mt-1.5">{verifierErr}</p>}
                </div>
                {addVerPending ? (
                  <div className="w-full py-3 rounded-2xl text-center text-sm font-semibold text-white/60 border border-white/10" style={{ background: "rgba(255,255,255,0.04)" }}>Confirm in wallet…</div>
                ) : (
                  <button onClick={handleAddVerifier} className="w-full text-white py-3 rounded-2xl font-semibold text-sm"
                    style={{ background: "linear-gradient(135deg,#6366f1,#4f46e5)", boxShadow: "0 8px 24px rgba(99,102,241,0.3)" }}>
                    Add Verifier →
                  </button>
                )}
                {addVerOk && <div className="rounded-xl px-4 py-3 border border-green-500/20" style={{ background: "rgba(34,197,94,0.06)" }}><p className="text-green-400 text-sm font-semibold">✅ Verifier added</p></div>}
              </div>
