...
import { CONTRACTS, CREDENTIAL_REGISTRY_ABI, CREDENTIAL_TIER, type TierKey } from "../../../lib/contracts";
...
function computeTrust(credentials: LoadedCredential[]) {
  ...
  const institutionVerif  = loaded.filter((c) => !c.onChain!.isRevoked && c.onChain!.tier === 1).length;
  const selfAttested      = loaded.filter((c) => !c.onChain!.isRevoked && c.onChain!.tier === 2).length;
  const total             = credentials.length;
  // Score: institution=1.0, self-attested=0.5, revoked=0
  const score = total > 0
    ? Math.round(((institutionVerif * 1.0 + selfAttested * 0.5) / total) * 100)
    : 0;
  return { revoked, institutionVerif, selfAttested, total, score };
}
...
function TrustBar({ score, stats }: {
  score: number;
  stats: { institutionVerif: number; selfAttested: number; revoked: number; total: number };
}) {
    ...
    <div className="rounded-2xl border border-white/[0.08] p-5"
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="text-center">
          <p className="text-green-400 text-lg font-black">{stats.institutionVerif}</p>
          <p className="text-white/30 text-xs leading-tight">Institution<br />Verified</p>
        </div>
        <div className="text-center">
          <p className="text-amber-400 text-lg font-black">{stats.selfAttested}</p>
          <p className="text-white/30 text-xs leading-tight">Self<br />Attested</p>
        </div>
        <div className="text-center">
          <p className={`text-lg font-black ${stats.revoked > 0 ? "text-red-400" : "text-white/20"}`}>
            {stats.revoked}
          </p>
          <p className="text-white/30 text-xs leading-tight">Revoked</p>
        </div>
      </div>
    </div>
  );
            ...
            <div>
              ...
              {!isRevoked && (
                <div className={`inline-flex items-center gap-1 text-xs font-semibold mt-0.5 ${
                  tier === 1 ? "text-green-400" : "text-amber-400"
                }`}>
                  {tier === 1 ? "✅" : "✍️"} {tierInfo?.label ?? "Unknown"}
                </div>
              )}
              {isRevoked && (
          ...
          <div className="mt-3 pt-3 border-t border-white/[0.05] space-y-1.5">
            ...
            {onChain?.candidate && (
              <div className="flex gap-3">
                ...
                <a href={`https://testnet.qie.digital/address/${onChain.candidate}`}
                  target="_blank" rel="noopener noreferrer"
            ...
            <div className="flex gap-3">
              ...
              <span className="text-white/40 text-xs">QIE Testnet · Chain ID 1983</span>
            </div>
            {details?.documentCID && (
              <div className="flex gap-3">
...
export default function PortfolioPage({ params }: { params: { cid: string } }) {
  ...
  // ── Step 1: Fetch the bundle from IPFS ──────────────────────────────────
  useEffect(() => {
    if (!cid) return;
  ...
  // ── Step 2: Fetch on-chain + metadata for each credential ───────────────
  useEffect(() => {
        ...
        <div className="rounded-3xl border border-white/[0.08] p-8 mb-6"
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <a href={`https://testnet.qie.digital/address/${bundle.creatorWallet}`}
                target="_blank" rel="noopener noreferrer"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-white/[0.06] p-5"
              ...
              <a href={`https://testnet.qie.digital/address/${CONTRACTS.CREDENTIAL_REGISTRY}`}
                target="_blank" rel="noopener noreferrer"
