...
/// @notice Core credential issuance, verification, and revocation logic
contract CredentialRegistry is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    struct Credential {
        ...
        string  ipfsCID;            // Pinata CID of encrypted offchain data
        address issuer;             // institution wallet (or candidate for Tier 2)
        string  issuerPassDID;      // QIE Pass DID of issuer
        address candidate;
        string  candidatePassDID;   // QIE Pass DID of candidate (optional)
        Tier    tier;
        uint256 issuedAt;
        bool    isRevoked;
        string  revokeReason;
        uint256 nftTokenId;         // linked soulbound NFT
    }

    // credentialId => Credential
    mapping(bytes32 => Credential) public credentials;

    // candidate wallet => list of their credentialIds
    mapping(address => bytes32[]) public candidateCredentials;

    // track all credential IDs
    ...
    /// @notice Institution issues a Tier 1 verified credential
    function issueCredential(
        ...
        // Get institution QIE Pass DID (only if QIE Pass is deployed)
        ...
        // Mint soulbound NFT for candidate — CORE QIE ecosystem use
    ...
    /// @notice Candidate self-attests an existing credential (Tier 2)
    ...
    /// @notice Institution upgrades a Tier 2 credential to Tier 1
    /// @dev Hash must match — institution confirms existing self-attested doc
    ...
    /// @notice Revoke a credential — reason required, stored permanently onchain
    ...
    /// @notice Reissue credential to new wallet (e.g. lost private key)
    ...
    /// @notice Main verify function — returns full credential status
